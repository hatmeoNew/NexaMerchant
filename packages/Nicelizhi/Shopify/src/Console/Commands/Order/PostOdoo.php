...
class PostOdoo extends Command
{
    ...
    public function postOrder($id)
    {
        ...
        try {
            ...
            if ($response->getStatusCode() == 200) {
                ...
                try {
                    ...
                    if (!empty($response_data['success']) && $response_data['success'] == true) {
                        ...
                        Utils::sendFeishuErp($response_data['message'] . ' post failed --order_id=' . $id . ' website:' . $webSiteName, 'red');
                        return false;
                    }
                } catch (\Throwable $th) {
                    echo $th->getMessage(), PHP_EOL;
                    Utils::sendFeishuErp($th->getMessage(). ' --order_id=' . $id . ' website:' . $webSiteName, 'red');
                    return false;
                }
            }
            ...
            Utils::sendFeishuErp($message, 'red');
            echo $message;
            return false;
        } catch (ClientException $e) {
            var_dump($e->getMessage());
            Log::error(json_encode($e->getMessage()));
            Utils::sendFeishuErp($e->getMessage() . '--' . $id . " fix check it ", 'red');
